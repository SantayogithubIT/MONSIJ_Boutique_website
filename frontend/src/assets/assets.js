import katan1 from './katan1.png'
import pic2 from './pic2.jpg'
import pic3 from './pic3.jpg'
import pic4 from './pic4.jpg'
import pic5 from './pic5.jpg'
import pic6 from './pic6.jpg'
import pic7 from './pic7.jpg'
import pic8 from './pic8.jpg'
import pic9 from './pic9.jpg'
import pic10 from './pic10.jpg'
import pic11 from './pic11.jpg'
import pic12 from './pic12.jpg'
import pic13 from './pic13.jpg'
import pic14 from './pic14.jpg'
import pic15 from './pic15.jpg'
import pic16 from './pic16.jpg'
import pic17 from './pic17.jpg'
import pic18 from './pic18.jpg'
import pic19 from './pic19.jpg'
import pic20 from './pic20.jpg'
import pic21 from './pic21.jpg'
import pic22 from './pic22.jpg'
import pic23 from './pic23.jpg'
import pic24 from './pic24.jpg'
import pic25 from './pic25.jpg'
import pic26 from './pic26.jpg'
import pic27 from './pic27.jpg'
import pic28 from './pic28.jpg'
import pic29 from './pic29.jpg'
import pic30 from './pic30.jpg'
import pic31 from './pic31.jpg'
import pic32 from './pic32.jpg'
import pic33 from './pic33.jpg'
import pic34 from './pic34.jpg'
import pic35 from './pic35.jpg'
import pic36 from './pic36.jpg'
import pic37 from './pic37.jpg'
import pic38 from './pic38.jpg'
import pic39 from './pic39.jpg'
import pic40 from './pic40.jpg'
import imon_bhaduri from './imon_bhaduri.jpg'


import logo from './logo.png'
import hero from './hero.jpg'
import cart_icon from './cart_icon.png'
import bin_icon from './bin_icon.png'
import dropdown_icon from './dropdown_icon.png'
import exchange_icon from './exchange_icon.png'
import profile_icon from './profile_icon.png'
import quality_icon from './quality_icon.png'
import search_icon from './search_icon.png'
import star_dull_icon from './star_dull_icon.png'
import star_icon from './star_icon.png'
import support_img from './support_img.png'
import menu_icon from './menu_icon.png'
import about_img from './about_img.png'
import contact_img from './contact_img.png'
import razorpay_logo from './razorpay_logo.png'
import stripe_logo from './stripe_logo.png'
import cross_icon from './cross_icon.png'
import admin from './admin.jpg'
import hero2 from './hero2.png'
import Mon_logo from './Mon_logo.png'

export const assets = {
    logo,
    hero,
    cart_icon,
    dropdown_icon,
    exchange_icon,
    profile_icon,
    quality_icon,
    search_icon,
    star_dull_icon,
    star_icon,
    bin_icon,
    support_img,
    menu_icon,
    about_img,
    contact_img,
    razorpay_logo,
    stripe_logo,
    cross_icon,
    admin,
    Mon_logo,
    imon_bhaduri,
    hero2
}


export const products = [
    {
        _id: "aaaaa",
        name: "Women Round Neck Cotton Top",
        description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
        price: 100,
        image: [katan1, pic2, pic3, pic4, pic5],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["One Size"],
        date: 1716634345448,
        bestseller: true
    },

   { _id: "aaaab",
    name: "Men Cotton Shirt",
    description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
    price: 200,
    image: [pic6, pic7, pic8, pic9, pic10],
    category: "Men",
    subCategory: "Topwear",
    sizes: ["M", "L", "XL"],
    date: 1716621345448,
    bestseller: false
   },
   {
    _id: "aaaad",
    name: "Men Round Neck Pure Cotton T-shirt",
    description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
    price: 110,
    image: [pic11, pic12, pic13, pic14, pic15],
    category: "Men",
    subCategory: "Topwear",
    sizes: ["S", "M", "XXL"],
    date: 1716621345448,
    bestseller: true
},
   {
    _id: "aaaed",
    name: "সহজ পাঠ শাড়ি",
    description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
    price: 1500,
    image: [pic16, pic17, pic18, pic19, pic20],
    category: "Women",
    subCategory: "Topwear",
    sizes: ["One Size"],
    date: 1716621345448,
    bestseller: true
},
   {
    _id: "aaaef",
    name: "Middle check Mangalgiri sarees",
    description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
    price: 1600,
    image: [pic21, pic22, pic23, pic24, pic25],
    category: "Women",
    subCategory: "Topwear",
    sizes: ["One Size"],
    date: 1716621345448,
    bestseller: true
},
   {
    _id: "aaagf",
    name: "Cotton Ikkat sarees",
    description: "A lightweight, usually knitted, pullover shirt, close-fitting and with a round neckline and short sleeves, worn as an undershirt or outer garment.",
    price: 1000,
    image: [pic26, pic27, pic28, pic29, pic30],
    category: "Women",
    subCategory: "Topwear",
    sizes: ["One Size"],
    date: 1716621345448,
    bestseller: true
},
   {
    _id: "aabgf",
    name: "Kashmiri Kurta",
    description: "Hand embroidery",
    price: 2000,
    image: [pic31, pic32, pic33, pic34, pic35],
    category: "Women",
    subCategory: "Topwear",
    sizes: ["40","41","42" ,"43","44","45","46"],
    date: 1716621345448,
    bestseller: false
},
   {
    _id: "aabwf",
    name: "Readymade hakoba shiuli saree",
    description: "Hand embroidery",
    price: 3000,
    image: [pic36, pic37, pic38, pic39, pic40],
    category: "Kids",
    subCategory: "Topwear",
    sizes: ["One Size"],
    date: 1716621345448,
    bestseller: false
}
]